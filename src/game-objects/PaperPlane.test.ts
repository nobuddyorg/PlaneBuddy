    ...
    describe('updatePaperPlane', () => {
        ...
        it('should apply lift when pointer is down and above the paperPlane', (done) => {
            game.events.on('create', () => {
                ...
                (paperPlane.body as Phaser.Physics.Arcade.Body).velocity.y = initialVelocityY;
                ...
                expect(paperPlane.body.velocity.y).toBeCloseTo(initialVelocityY - expectedLift);
                done();
            });
        });
        ...
        it('should apply sink force when pointer is down and below the paperPlane', (done) => {
            game.events.on('create', () => {
                ...
                expect(paperPlane.body.velocity.y).toBe(initialVelocityY + FlightConstants.SINK_FORCE);
                done();
            });
        });
        ...
        it('should tumble when velocity is below stall speed', (done) => {
            game.events.on('create', () => {
                const paperPlane = scene.paperPlane;
                (paperPlane.body as Phaser.Physics.Arcade.Body).velocity.x = FlightConstants.STALL_SPEED - 10;

                paperPlane.updatePaperPlane();

                expect((paperPlane.body as Phaser.Physics.Arcade.Body).angularVelocity).toBe(FlightConstants.TUMBLE_ANGULAR_VELOCITY);
                done();
            });
        });
        ...
        it('should fly normally and set angle based on velocity when above stall speed', (done) => {
            game.events.on('create', () => {
                ...
                expect((paperPlane.body as Phaser.Physics.Arcade.Body).angularVelocity).toBe(0);
