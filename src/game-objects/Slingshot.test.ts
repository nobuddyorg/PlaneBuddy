...
describe("Slingshot", () => {
  ...
  it("should store the drag start position on pointerdown", () => {
    const pointer = {
      position: new Phaser.Math.Vector2(100, 150),
  ...
  it("should not launch if pointerup is fired without a pointerdown", () => {
    const pointer = {
      position: new Phaser.Math.Vector2(200, 250),
    } as Phaser.Input.Pointer;
  ...
  it("should calculate launch velocity and launch the paperPlane on pointerup", () => {
    ...
    pointerDownCallback.call(slingshot, {
      position: startPos,
    } as Phaser.Input.Pointer);
    pointerUpCallback.call(slingshot, {
      position: endPos,
    } as Phaser.Input.Pointer);

    const dragVector = endPos.clone().subtract(startPos);
    const expectedVelocity = dragVector
      .clone()
      .scale(SlingshotConstants.VELOCITY_MULTIPLIER);
  ...
  it("should cap the launch velocity if the drag distance exceeds the maximum", () => {
    ...
    const endPos = new Phaser.Math.Vector2(
      100,
      100 + SlingshotConstants.MAX_DRAG_DISTANCE + 50,
    );
  ...
  it("should reset the drag start position after launching", () => {
    ...
    pointerUpCallback.call(slingshot, {
      position: new Phaser.Math.Vector2(120, 170),
    } as Phaser.Input.Pointer);
    ...
    vi.clearAllMocks();
    pointerUpCallback.call(slingshot, {
      position: new Phaser.Math.Vector2(130, 180),
    } as Phaser.Input.Pointer);
    expect(paperPlane.launch).not.toHaveBeenCalled();
  });
});
